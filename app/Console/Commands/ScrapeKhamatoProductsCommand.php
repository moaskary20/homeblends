...
class ScrapeKhamatoProductsCommand extends Command
{
    protected $signature = 'products:scrape-khamato
                            {--collections=* : Collection handles (default: all Khamato sections)}
                            {--limit=5 : Max products per collection}
                            {--dry-run : Preview without saving}
                            {--no-images : Skip image download}
                            {--skip-ping : Skip connectivity check before scrape}';

    protected $description = 'Scrape accessory products from khamato.com';

    public function handle(KhamatoScraperService $scraper, ScrapedProductImporter $importer): int
    {
        if (! $this->option('skip-ping')) {
            $this->line('Checking connectivity to khamato.com…');
            if (! $scraper->ping()) {
                $this->warn('Ping failed — continuing anyway (use --skip-ping to hide this check).');
            } else {
                $this->info('Connected to khamato.com');
            }
        }
