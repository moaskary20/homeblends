...
class ScrapeRayaProductsCommand extends Command
{
    protected $signature = 'products:scrape-raya
                            {--collections=* : Collection handles (default: all Raya sections)}
                            {--limit=5 : Max products per collection}
                            {--dry-run : Preview without saving}
                            {--no-images : Skip image download}
                            {--skip-ping : Skip connectivity check before scrape}';

    protected $description = 'Scrape home appliances from rayashop.com';

    public function handle(RayaScraperService $scraper, ScrapedProductImporter $importer): int
    {
        if (! $this->option('skip-ping')) {
            $this->line('Checking connectivity to rayashop.com…');
            if (! $scraper->ping()) {
                $this->warn('Ping failed — continuing anyway (use --skip-ping to hide this check).');
            } else {
                $this->info('Connected to rayashop.com');
            }
        }
