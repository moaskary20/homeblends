...
class ScrapeGemmaProductsCommand extends Command
{
    protected $signature = 'products:scrape-gemma
                            {--collections=* : Collection handles (default: all Gemma sections)}
                            {--limit=5 : Max products per collection}
                            {--dry-run : Preview without saving}
                            {--no-images : Skip image download}
                            {--skip-ping : Skip connectivity check before scrape}';

    protected $description = 'Scrape ceramic products from gemma.com.eg/shop';

    public function handle(GemmaScraperService $scraper, ScrapedProductImporter $importer): int
    {
        if (! $this->option('skip-ping')) {
            $this->line('Checking connectivity to gemma.com.eg…');
            if (! $scraper->ping()) {
                $this->warn('Ping failed — continuing anyway (use --skip-ping to hide this check).');
            } else {
                $this->info('Connected to gemma.com.eg API');
            }
        }
