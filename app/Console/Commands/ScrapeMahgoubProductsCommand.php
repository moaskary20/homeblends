...
class ScrapeMahgoubProductsCommand extends Command
{
    protected $signature = 'products:scrape-mahgoub
                            {--collections=* : Collection handles (default: all Mahgoub sections)}
                            {--limit=5 : Max products per collection}
                            {--dry-run : Preview without saving}
                            {--no-images : Skip image download}
                            {--skip-ping : Skip connectivity check before scrape}';

    protected $description = 'Scrape ceramic products from mahgoub.com';

    public function handle(MahgoubScraperService $scraper, ScrapedProductImporter $importer): int
    {
        if (! $this->option('skip-ping')) {
            $this->line('Checking connectivity to mahgoub.com…');
            if (! $scraper->ping()) {
                $this->warn('Ping failed — continuing anyway (use --skip-ping to hide this check).');
            } else {
                $this->info('Connected to mahgoub.com');
            }
        }
