...
class ScrapeCleopatraProductsCommand extends Command
{
    protected $signature = 'products:scrape-cleopatra
                            {--collections=* : Collection handles (default: all Cleopatra sections)}
                            {--limit=5 : Max products per collection}
                            {--dry-run : Preview without saving}
                            {--no-images : Skip image download}
                            {--skip-ping : Skip connectivity check before scrape}';

    protected $description = 'Scrape ceramic products from cleopatraceramics.store';

    public function handle(CleopatraScraperService $scraper, ScrapedProductImporter $importer): int
    {
        if (! $this->option('skip-ping')) {
            $this->line('Checking connectivity to cleopatraceramics.store…');
            if (! $scraper->ping()) {
                $this->warn('Ping failed — continuing anyway (use --skip-ping to hide this check).');
            } else {
                $this->info('Connected to Cleopatra Ceramics API');
            }
        }
