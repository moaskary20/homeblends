...
class AriikaScraperService
{
    ...
    protected array $furnitureCollections;
    ...
    {
        $config = config('product-scraper.ariika');
        ...
        $this->furnitureCollections = $config['furniture_collections'];
     ...
     */
    public function scrapeFurniture(array $collectionHandles, int $maxPerCollection = 10): Collection
    {
        $products = collect();
        $this->scrapeErrors = collect();

        foreach ($collectionHandles as $handle) {
            if (! isset($this->furnitureCollections[$handle])) {
                $this->scrapeErrors->push([
                    'handle' => $handle,
                    'message' => __('ecommerce.scrape_unknown_collection', ['handle' => $handle]),
                ]);

                continue;
            }

            $categoryName = $this->furnitureCollections[$handle];
    ...
    public function fetchCollectionProducts(string $handle, int $limit = 10): Collection
    {
        ...
        if (! $response->successful()) {
            throw new \RuntimeException(__('ecommerce.scrape_collection_failed', [
                'handle' => $handle,
    ...
    public function mapShopifyProduct(array $product, string $collectionHandle, string $categoryName): array
    {
        ...
        return [
            ...
            'vendor' => (string) ($product['vendor'] ?? 'ariika'),
    ...
    {
        try {
            $response = $this->http()->get("{$this->baseUrl}/collections/indoor-furniture/products.json", [
                'limit' => 1,
            ]);
